...
```mermaid
flowchart TD
    A["C-Phycocyanin enters cancer cell"] --> B["Localises in mitochondria"]
    B --> C["Drops mitochondrial membrane potential (MMP)"]
    C --> D["Cytochrome c released from mitochondria"]
    D --> E["Caspase-9 activated (initiator)"]
    E --> F["Caspase-3 activated (executioner)"]
    F --> G["PARP cleavage → DNA fragmentation"]
    G --> H["APOPTOSIS — cancer cell dies"]
    
    B --> I["Shifts Bcl-2/Bax ratio"]
    I --> J["↓ Bcl-2 (anti-death protein)"]
    I --> K["↑ Bax (pro-death protein)"]
    J --> D
    K --> D
    
    B --> L["Generates ROS inside cancer cell"]
    L --> D
    
    style H fill:#2d5016,color:#fff,stroke:#1a3a0a
...
```mermaid
flowchart LR
    subgraph Chlorella["🟢 CHLORELLA"]
        direction TB
        C1["Heavy metal chelation"]
        C2["CGF → DNA repair"]
        C3["Liver detox support"]
        C4["Immune activation"]
        C5["Carcinogen binding"]
    end
    
    subgraph Spirulina["🔵 SPIRULINA"]
        direction TB
        S1["Mitochondrial apoptosis"]
        S2["Cell cycle arrest"]
        S3["Anti-angiogenesis"]
        S4["COX-2 inhibition"]
        S5["mTOR pathway shutdown"]
    end
    
    Chlorella -->|"Cleans the terrain"| Body["Reduced toxic burden\n+ Active tumour targeting"]
    Spirulina -->|"Targets the tumour"| Body
    
    style Chlorella fill:#1a5a1a,color:#fff
    style Spirulina fill:#1a3a6a,color:#fff
    style Body fill:#4a1a4a,color:#fff
```

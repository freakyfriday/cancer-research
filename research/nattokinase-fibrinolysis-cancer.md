...
```mermaid
flowchart TD
    NK["Nattokinase (oral)"] --> D["Direct fibrin hydrolysis"]
    NK --> T["↑ tPA release from endothelium"]
    NK --> U["Prourokinase → uPA activation"]
    NK --> P["PAI-1 degradation"]
    
    T --> PL["Plasminogen → PLASMIN"]
    U --> PL
    P -->|"Removes brake"| PL
    D --> FD["FIBRIN DISSOLVED"]
    PL --> FD
    
    FD --> I["Immune cells reach tumour"]
    FD --> DR["Drug delivery improved"]
    FD --> O2["Oxygen reaches tumour core"]
    FD --> META["Metastasis scaffold removed"]
    
    style NK fill:#1a4a7a,color:#fff
    style FD fill:#2d5016,color:#fff,stroke:#1a3a0a
